...
class ArxivPaper:
    ...
    def article(self) -> str:
        ...
        if paper_type == PaperType.SOLUTION_TYPE:
            # 解决方案型论文的提示词
            system_prompt = "你是一个专业的学术分析师，请仔细阅读这篇解决方案型论文，并按照以下结构抽取论文内容：需要解决的问题、现有方案的缺点、新方案的创新点。每个部分都要简明扼要，总字数控制在500字左右。"
...
（突出论文提出的新方法的创新之处，需要详细说明）"""
            ...
            # 探究型论文的提示词
            system_prompt = "你是一个专业的学术分析师，请仔细阅读这篇探究型论文，并按照以下结构抽取论文内容：探究的问题、实验结论。每个部分都要简明扼要，总字数控制在400字左右。"
            user_prompt = f"""论文标题：{self.title}
